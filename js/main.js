...
let menuToggle, sideMenu, closeMenu, notificationBtn, notificationPanel, closeNotifications;
let searchInput, productModal, cartModal, loginModal, cartBtn, whatsappBtn, supportBtn;
...
function createProductCard(product) {
    ...
    if (product.images && product.images[0]) {
        ...
        const categoryImages = {
            'broilers': 'https://placehold.co/400x300/FF6B00/white?text=Broilers',
            'layers': 'https://placehold.co/400x300/FF6B00/white?text=Layers',
            'eggs': 'https://placehold.co/400x300/FF6B00/white?text=Fresh+Eggs',
            'chicks': 'https://placehold.co/400x300/FF6B00/white?text=Chicks',
            'other': 'https://placehold.co/400x300/FF6B00/white?text=KUKU+YETU'
        };
        imageUrl = categoryImages[product.category] || 'https://placehold.co/400x300/FF6B00/white?text=KUKU+YETU';
    }
        ...
        <div class="product-card" data-product-id="${product.id}">
            <div class="product-image">
                <img data-src="${imageUrl}" 
                     ...
                     class="lazy-image"
                     onerror="this.onerror=null; this.src='https://placehold.co/400x300/FF6B00/white?text=KUKU+YETU'">
                <span class="product-category">${product.category || 'Uncategorized'}</span>
                <span class="stock-status ${stockClass}">${stockText}</span>
            </div>
            <div class="product-info">
                <h3 class="product-title">${escapeHtml(product.title) || 'Untitled'}</h3>
                <p class="product-description">${escapeHtml(product.description ? product.description.substring(0, 60) + '...' : 'No description')}</p>
...
function renderProductModal(product) {
            ...
            <div class="product-images">
                <div class="image-slider">
                    <div class="slider-container" id="imageSlider">
                        ${product.images.map(img => `<img src="${getSafeImageUrl(img)}" alt="${product.title}" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\'%3E%3Ctext y=\'.9em\' font-size=\'90\'%3E🐔%3C/text%3E%3C/svg%3E'">`).join('')}
                    </div>
    ...
    modalBody.innerHTML = `
        <div class="product-detail">
            ${imagesHtml || '<div class="product-detail-image"><img src="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\'%3E%3Ctext y=\'.9em\' font-size=\'90\'%3E🐔%3C/text%3E%3C/svg%3E" alt="No image"></div>'}
            <h2 class="product-full-title">${escapeHtml(product.title)}</h2>
            ...
            <p class="full-description">${escapeHtml(product.description || 'No description available')}</p>
...
function renderCart() {
    ...
    cartBody.innerHTML = `
        <div class="cart-items" style="max-height: 400px; overflow-y: auto;">
            ${cart.map(item => {
                const cartImageUrl = getSafeImageUrl(item.image);
                return `
        ...
        <div class="profile-page" style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: white; border-radius: 10px; padding: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <div style="text-align: center; margin-bottom: 30px;">
                    ...
                    <h3 style="font-size: 24px; margin-bottom: 5px;">${escapeHtml(currentUser.full_name)}</h3>
                    <p style="color: #666;">Member since ${new Date().toLocaleDateString()}</p>
                </div>
                
                <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px;">
                    <p style="margin: 10px 0;"><strong><i class="fas fa-envelope" style="color: var(--primary-color); margin-right: 10px;"></i> Email:</strong> ${escapeHtml(currentUser.email)}</p>
                    <p style="margin: 10px 0;"><strong><i class="fas fa-phone" style="color: var(--primary-color); margin-right: 10px;"></i> Phone:</strong> ${escapeHtml(currentUser.phone)}</p>
                </div>
